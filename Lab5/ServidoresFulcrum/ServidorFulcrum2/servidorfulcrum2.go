...
func (s *FulcrumServer) connectToServers() {
	for i, addr := range []string{"dist061.inf.santiago.usm.cl:50051", "dist063.inf.santiago.usm.cl:50053"} {
		for {
			serverConn, err := grpc.Dial(addr, grpc.WithInsecure())
			if err != nil {
				log.Printf("Could not connect to Fulcrum Server %d: %v. Retrying...", 2*i+1, err)
				time.Sleep(5 * time.Second)
				continue
			}
			s.serverClients = append(s.serverClients, pb.NewServersClient(serverConn))
			log.Printf("Connected to Fulcrum Server %d", 2*i+1)
			break
		}
	}
}
...
func (s *FulcrumServer) startPropagation() {
	for {
		...
		for i, client := range s.serverClients {
			...
			if err != nil {
				log.Fatalf("Could not propagate changes to Fulcrum Server %d: %v", 2*i+1, err)
			}
		}
		s.mu.Lock()
...
func main() {
	...
	log.Printf("Fulcrum Server %d is running on port 50052\n", s.id+1)
